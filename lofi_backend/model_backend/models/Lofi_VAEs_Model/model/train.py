...
def train(dataset, model, name):
    ...
    while epoch < max_epochs:
        ...
        axs[0, 0].plot(epochs, [loss_tensor.detach().cpu().numpy() for loss_tensor in train_losses_chords], label='Train', color='royalblue')
        axs[0, 0].plot(epochs, [loss_tensor.detach().cpu().numpy() for loss_tensor in val_losses_chords], label='Val', color='royalblue', linestyle='dotted')
        ...
        axs[0, 1].plot(epochs, [loss_tensor.detach().cpu().numpy() for loss_tensor in train_losses_melodies], label='Train', color='royalblue')
        axs[0, 1].plot(epochs, [val_loss.detach().cpu().numpy() for val_loss in val_losses_melodies], label='Val', color='royalblue', linestyle='dotted')
