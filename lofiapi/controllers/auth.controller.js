...
export const google = async (req, res, next) => {
    try{
        ...
        if(user){
            ...
            const newUser = new User({username: req.body.name.split(" ").join("").toLowerCase() + (Math.floor(Math.random() * 10000)).toString(), email: req.body.email, password: hashedPassword, profilePicture: req.body.profilePicture});
            await newUser.save();
